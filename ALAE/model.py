...
class Model(nn.Module):
    ...
    def generate(self, lod, blend_factor, z=None, count=32, mixing=True, noise=True, return_styles=False, no_truncation=False):
        device = self.dlatent_avg.buff.device
        dtype = self.dlatent_avg.buff.dtype
        if z is None:
            z = torch.randn(count, self.latent_size, device=device, dtype=dtype)
        else:
        ...
        if mixing and self.style_mixing_prob is not None:
            if random.random() < self.style_mixing_prob:
                z2 = torch.randn(count, self.latent_size, device=device, dtype=dtype)
                styles2 = self.mapping_f(z2)[:, 0]
                styles2 = styles2.view(styles2.shape[0], 1, styles2.shape[1]).repeat(1, self.mapping_f.num_layers, 1)

                layer_idx = torch.arange(self.mapping_f.num_layers, device=styles.device)[np.newaxis, :, np.newaxis]
                cur_layers = (lod + 1) * 2
        ...
        if (self.truncation_psi is not None) and not no_truncation:
            layer_idx = torch.arange(self.mapping_f.num_layers, device=styles.device)[np.newaxis, :, np.newaxis]
            ones = torch.ones(layer_idx.shape, dtype=styles.dtype, device=styles.device)
    ...
    def forward(self, x, lod, blend_factor, d_train, ae):
        if ae:
            ...
            z = torch.randn(x.shape[0], self.latent_size, device=x.device, dtype=x.dtype)
...
class GenModel(nn.Module):
    ...
    def generate(self, lod, blend_factor, z=None):
        ...
        dtype = self.dlatent_avg.buff.dtype
        if z is None:
            z = torch.randn(32, self.latent_size, device=device, dtype=dtype)
        else:
            z = z.to(device=device, dtype=dtype)
        styles = self.mapping_f(z)[:, 0]
        s = styles.view(styles.shape[0], 1, styles.shape[1])
