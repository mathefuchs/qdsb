...
import numpy as np
import torch
from cell_exp.common import subsample_points
    ...
    *,
    ...
    for x_start in range(0, x.shape[0], chunk_size):
        ...
        for y_start in range(0, y.shape[0], chunk_size):
            ...
            sq_dists = (x_norm + y_norm - 2.0 * x_chunk @
                        y_chunk.T).clamp_min_(0.0)
...
def compute_mmd(
    ...
    *,
    ...
    predicted = subsample_points(predicted, max_points).to(
        dtype=torch.float32, device="cpu")
    ground_truth = subsample_points(ground_truth, max_points).to(
        dtype=torch.float32,
        device="cpu",
    )
    n = predicted.shape[0]
    ...
    if n == 0 or m == 0:
        raise ValueError(
            "MMD requires non-empty predicted and reference samples.")

    sum_xx = _rbf_kernel_sum(predicted, predicted,
                             gamma=gamma, chunk_size=chunk_size)
    sum_yy = _rbf_kernel_sum(
        ground_truth,
        ground_truth,
        gamma=gamma,
        chunk_size=chunk_size,
    )
    sum_xy = _rbf_kernel_sum(
        predicted,
        ground_truth,
        gamma=gamma,
        chunk_size=chunk_size,
...
class QualityCheckpointRecorder:
    ...
    def time_evaluation(
        ...
        *,
        ...
        metric_value = float(evaluate())
        self.cumulative_eval_seconds += time.perf_counter() - eval_start_time
...
class UpdateQualityCheckpointSchedule:
    ...
    def observe_update(self, completed_updates: int) -> int | None:
        ...
        final_due = completed_updates == self.total_updates
        if not interval_due and not final_due:
            return None

        while completed_updates >= self.next_checkpoint_update:
            self.next_checkpoint_update += (
                self.eval_every_epochs * self.steps_per_epoch
            )

        return max(
            1,
            int(np.ceil(completed_updates / self.steps_per_epoch)),
        )
...
def summarize_quality_curve(
    ...
    *,
    ...
    for seed_result in seed_results:
        for leave_out_entry in seed_result["leave_out"]:
            for checkpoint in leave_out_entry.get("quality_curve", []):
                epoch = int(checkpoint["epoch"])
                bucket = checkpoints_by_epoch.setdefault(
                    epoch,
                    {"elapsed_seconds": [], metric_key: []},
                )
                bucket["elapsed_seconds"].append(
