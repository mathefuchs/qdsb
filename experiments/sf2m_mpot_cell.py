"""Reproduce the SF2M cell benchmark with m-POT couplings.

This script mirrors ``experiments/sf2m_cell.py`` exactly except for the
mini-batch coupling sampler used during Schrödinger bridge matching training:
minibatch OT is replaced by mini-batch partial OT (m-POT) following
...
class MPOTPlanSampler:
    """Sample pairs from a minibatch partial OT plan."""
    ...
    def __init__(
        ...
        *,
        ...
        if not (0.0 < transport_fraction <= 1.0):
            raise ValueError(
                f"transport_fraction must lie in (0, 1], got {transport_fraction}."
            )
        if method not in {"exact", "sinkhorn"}:
            raise ValueError(
                f"Unsupported m-POT method {method!r}; expected 'exact' or 'sinkhorn'."
            )
        self.method = method
    ...
    def get_map(self, x0: torch.Tensor, x1: torch.Tensor) -> np.ndarray:
        ...
        if not np.all(np.isfinite(plan)):
            if self.warn:
                warnings.warn(
                    "Numerical errors in m-POT plan, reverting to uniform plan.",
                    stacklevel=2,
                )
...
class SchrodingerBridgeMPOTConditionalFlowMatcher(
    SchrodingerBridgeConditionalFlowMatcher
):
    def __init__(
        self,
...
def train_leave_one_out_sf2m_mpot(
    ...
    *,
    ...
    for seed in seeds:
        ...
        for missing_index in spec.leave_out:
            ...
            flow_model, score_model = train_leave_one_out_sf2m_mpot(
                timepoints,
                model_times,
                missing_index=missing_index,
                sigma=sigma,
                ot_method=ot_method,
                mpot_fraction=mpot_fraction,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reproduce the SF2M cell benchmark with m-POT minibatch couplings.",
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
    ...
    parser.add_argument(
        "--mpot-fraction",
        type=float,
        default=DEFAULT_MPOT_FRACTION,
        help=(
            "Transported mass fraction s used by minibatch partial OT. "
            "s=1 recovers minibatch OT."
        ),
    )
    parser.add_argument(
        "--time-mode",
    ...
    parser.add_argument(
        ...
        help="Partial OT solver used during training.",
    )
...
def main() -> None:
    ...
    print(
        "Using SF2M with m-POT minibatch couplings: "
        f"s={args.mpot_fraction}, method={args.ot_method}."
    )
    ...
    for dataset_key in args.datasets:
        ...
        result["mpot_fraction"] = args.mpot_fraction
        results.append(result)

    print("\nSF2M-mPOT summary (Table 4 style)")
